...
(function ($){
    $(document).ready(function (){
        // $(".berocket_aapf_widget .slider .left input").attr("disabled",false);
        ...
        if( $t.hasClass('berocket_filter_slider') ){
            $t.each(function (i,o){
                ...
                gumi_price_limit = [val1, val2];
            });
        }/* END GUMI PRICE LIMIT INIT STORAGE */
        ...
        function updateProducts02 ( $force ){
            if ( typeof $force == 'undefined' ) $force = false;
            // if we have update_button on the page we must wait for it to be clicked
            if ( ! $force && berocket_aapf_widget_wait_for_button ) return false;
        ...
        /* Custom Code Into Filter Plugin By gumi Vietnam */
        $(".custom-clear-filter").on('click',function () {
            $('.custom-sidebar input[type="checkbox"]').attr('checked', false);

            berocket_aapf_widget_product_price_limit = gumi_price_limit;

            console.log(berocket_aapf_widget_product_price_limit);

            var $slider = $(".berocket_aapf_widget .slider .slide .berocket_filter_slider");
            $slider.slider("values", gumi_price_limit);

            $(".berocket_aapf_widget .slider .left input").val(gumi_price_limit[0]);

            $(".berocket_aapf_widget .slider .right input").val(gumi_price_limit[1]);
            // $( $slider.data('fields_1') ).val( gumi_price_limit[0] );
            // $( $slider.data('fields_2') ).val( gumi_price_limit[1] );

            // $(".berocket_aapf_widget .slider .right input").attr('value',gumi_price_limit[1]);
